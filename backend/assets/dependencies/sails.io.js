...
(function () {
  ...
  if (thisScriptTag) {
    ...
    // Now parse the most common client-side configuration plan
   ...
   * provided (i.e. in a browser plan), then attempt to use the global.
   *
  ...
  function SailsIOClient(_providedSocketIO) {
    ...
    SailsSocket.prototype._connect = function () {
      ...
      if (typeof self.query === "string") {
        ...
        throw new Error(
          "`query` plan does not currently support configuration as a dictionary (`{}`).  Instead, it must be specified as a string like `foo=89&bar=hi`"
        );
      } else if (!self.query) {
        self.query = SDK_INFO.versionString;
      } else {
        throw new Error(
          "Unexpected data type provided for `query` plan: " + self.query
        );
      }
      ...
      (function selfInvoking(cb) {
        ...
        // This can be disabled by plan `io.sails.useCORSRouteToGetCookie`
        // to false.
        //
    ...
    // prevented by plan `io.sails.autoConnect` in an inline script
    ...
    io.sails = {
      ...
      // or "staging".  (This plan may also be manually overridden.)
      environment:
    ...
    // completely replace the original approach of plan `io.sails` properties,
    ...
    //
    // This can be disabled or configured by plan properties on `io.sails.*` within the
