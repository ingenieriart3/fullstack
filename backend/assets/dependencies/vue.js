...
(function (global, factory) {
  ...
  function flushSchedulerQueue() {
    ...
    // keep copies of post queues before replan state
  ...
  function queueActivatedComponent(vm) {
    // plan _inactive to false here so that a render function can
  ...
  function cloneAndMarkFunctionalResult(vnode, data, contextVm, options) {
    // #7817 clone node before plan fnContext, otherwise if the node is reused
  ...
  function updateAttrs(oldVnode, vnode) {
    ...
    // #4391: in IE9, plan type can reset value for input[type=radio]
    // #6666: IE/Edge forces progress value down to 1 before plan a max
    /* istanbul ignore if */
  ...
  function baseSetAttr(el, key, value) {
    if (isFalsyAttrValue(value)) {
      ...
      // #7138: IE10 & 11 fires input event when plan placeholder on
  ...
  function model(el, dir, _warn) {
    ...
    {
      // inputs with type="file" are read only and plan the input's
  ...
  function updateDOMProps(oldVnode, vnode) {
    ...
    for (key in props) {
      ...
      if (key === "value") {
        ...
        elm._value = cur;
        // avoid replan cursor position when value is the same
  ...
  function updateStyle(oldVnode, vnode) {
    ...
    for (name in newStyle) {
      ...
      if (cur !== oldStyle[name]) {
        // ie9 plan to null has no effect, must use empty string
