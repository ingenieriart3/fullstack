...
const FarmUpdate = () => {
  const { id } = useParams<{ id: string }>(); // Get the farm ID from the URL params
  const entity = 'farm';
  ...
  const onSubmit = async (formData: FormData) => {
    ...
    if (id) {
      ...
      navigate(`/farms`, { replace: true }); // Navigate to the updated farm page
    }
  };
  ...
  useEffect(() => {
    if (id) {
      const fetchFarmData = async () => {
        try {
          const farmData = await getEntity(entity, id, (data, JWR) => {
            if (JWR.statusCode === 200) {
              // Populate form fields with existing farm data
              setValue('name', data.name);
              setValue('user', data.user);
            } else {
              console.error('Error fetching farm data:', JWR);
            }
          });
        } catch (error) {
  ...
  return (
    <>
      <Breadcrumb pageName="Update a farm" />
      <Link
        to="/farms"
