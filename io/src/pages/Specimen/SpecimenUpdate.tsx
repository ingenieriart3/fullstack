...
const SpecimenUpdate = () => {
  const { id } = useParams<{ id: string }>(); // Get the specimen ID from the URL params
  ...
  const onSubmit = async (formData: FormData) => {
    ...
    if (id) {
      ...
      navigate(`/specimens`, { replace: true }); // Navigate to the updated specimen page
    }
  };
  ...
  useEffect(() => {
    if (id) {
      const fetchSpecimenData = async () => {
        try {
          const specimenData = await getEntity(entity, id, (data, JWR) => {
            if (JWR.statusCode === 200) {
              // Populate form fields with existing specimen data
              setValue('name', data.name);
              setValue('user', data.user);
              setValue('birth', data.birth); // Set value for birth field
            } else {
              console.error('Error fetching specimen data:', JWR);
            }
          });
        } catch (error) {
  ...
  return (
    <>
      <Breadcrumb pageName="Update a specimen" />
      <Link
        to="/specimens"
